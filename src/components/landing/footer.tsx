...
export function LandingFooter() {
  ...
  return (
    <footer id="footer" className="relative bg-[#050810]">
      ...
      <div
        aria-hidden
        className="absolute left-1/2 top-0 h-px w-3/4 -translate-x-1/2 bg-gradient-to-r from-transparent via-[#6C3EF4]/40 to-transparent"
      />
      {/* Top purple accent bar */}
      <div aria-hidden className="h-[2px] w-full bg-gradient-to-r from-[#6C3EF4] via-[#818cf8] to-[#6C3EF4]" />
        ...
        <div className="grid gap-12 md:grid-cols-[1.4fr_1fr_1fr]">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <span className="flex size-10 items-center justify-center rounded-xl border border-[#6C3EF4]/40 bg-[#6C3EF4]/15 font-bold text-lg text-white">
                G
              </span>
              <div>
            ...
            <address className="mt-6 space-y-2.5 text-sm not-italic text-white/40">
              <p className="flex items-start gap-2.5">
                <MapPin className="mt-0.5 size-4 shrink-0 text-[#818cf8]" aria-hidden />
                <span>{t("footerAddress")}</span>
              </p>
              <p className="flex items-center gap-2.5">
                <Mail className="size-4 shrink-0 text-[#818cf8]" aria-hidden />
                <a
            ...
            <ul className="space-y-3 text-sm">
              {secciones.map((s) => (
                <li key={s.href}>
                  ...
                  >
                    <span className="size-1 rounded-full bg-[#6C3EF4]/60" />
                    {s.label}
                  </a>
                </li>
              ))}
              ...
              {puertas.map((p) => (
                <li key={p.href}>
                  <Link
                    ...
                    className="flex items-center gap-2 text-white/40 transition-colors hover:text-white"
                  >
                    <span className="size-1 rounded-full bg-[#818cf8]/60" />
                    {p.label}
                  </Link>
                </li>
              ))}
