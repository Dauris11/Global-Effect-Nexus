...
export function Eventos({ eventos }: { eventos: EventoPublico[] }) {
  ...
  return (
    <section
      ...
      className="franja-clara relative bg-background py-24 md:py-32"
    >
      {/* Blue accent top bar */}
      <div
        aria-hidden
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          ...
          {eventos.length > 0 && (
            <div className="relative w-full md:w-72">
              ...
              className="w-full rounded-full border border-border bg-background py-2.5 pl-10 pr-4 text-sm font-medium text-foreground placeholder:text-muted-foreground transition-colors focus-visible:border-primary/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
              />
            </div>
          )}
        </div>
        ...
        {eventos.length === 0 ? (
          <div className="mx-auto mt-12 max-w-2xl rounded-2xl border border-border bg-card p-10 text-center">
          ...
          <>
            <ul ref={rejilla} className="mt-8 grid gap-4 md:grid-cols-2">
              {filtrados.map((e) => {
                ...
                return (
                  <li key={e.id}>
                    <article className="flex h-full items-center gap-5 rounded-2xl border border-border bg-card p-5 transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/35 hover:bg-primary/[0.04] hover:shadow-plana">
                      {/* Date block */}
                      <div
                        className={cn(
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <h3 className="mt-1.5 truncate font-display text-base font-semibold text-foreground">
                          {e.titulo}
                        </h3>
                        {e.ubicacion && (
            ...
            {filtrando && filtrados.length > 0 && (
              <p
                aria-live="polite"
                className="mt-6 font-mono text-[10px] uppercase tracking-[0.15em] text-muted-foreground"
              >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "border-primary bg-primary text-primary-foreground shadow-[0_0_20px_rgba(32,150,186,0.35)]"
